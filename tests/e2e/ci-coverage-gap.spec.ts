...
async function fetchProjectTasks(request: APIRequestContext, projectId: string): Promise<Task[]> {
  const tasksRes = await request.get("/api/tasks");
  const tasksJson = await expectOkJson<{ tasks: Task[] }>(tasksRes, "GET /api/tasks(project scan)");
  const tasks = Array.isArray(tasksJson.tasks) ? tasksJson.tasks : [];
  return tasks.filter((task) => task.project_id === projectId);
}

async function waitForProjectTask(
  request: APIRequestContext,
  projectId: string,
  finder: (tasks: Task[]) => Task | undefined,
  timeoutMs: number,
  ...
  throw new Error(
    `project 조건에 맞는 task를 찾지 못했습니다. project=${projectId}, recent=${lastTasks
      .slice(0, 10)
      .map((task) => `${task.id}:${task.title}:${task.status}`)
      .join(" | ")}`,
  );
...
test.describe("CI coverage gap expansion", () => {
  ...
  test("Task lifecycle + Department/Agent/Project CRUD + Chat + Directive + Report + Terminal", async ({ request }) => {
    ...
    try {
      ...
      cleanup.agentIds.push(leaderId);
      ...
      cleanup.agentIds.push(memberId);
      ...
      const directiveTask = await waitForProjectTask(
        request,
        projectId,
        (tasks) => tasks.find((task) => task.title.includes(`ci-directive-${seed}`)),
        90_000,
      );
      cleanup.taskIds.push(directiveTask.id);
  ...
  test("Settings/Stats/WebSocket/DecisionInbox + language setting persistence + input validation", async ({
    ...
    try {
      ...
      const settingsGet = await expectOkJson<{ settings: Record<string, unknown> }>(
        settingsGetRes,
        "GET /api/settings",
      );
