...
async function fetchProjectTasks(request: APIRequestContext, projectId: string): Promise<TaskRow[]> {
  ...
  const json = await expectOkJson<TasksResponse>(response, "GET /api/tasks(project scan)");
...
test.describe("CI manual assignment coverage", () => {
  ...
  test("신규 부서/팀원 추가 + 수동 배정(선택 팀원/팀장 fallback) 실행 검증", async ({ request }) => {
    ...
    try {
      ...
      cleanup.taskIds.push(subordinateTask.id);
      ...
      const leaderTask = await waitForTaskAssignment(
        request,
        leaderProjectId,
        leaderTaskTitle,
        leaderId,
      ...
      cleanup.taskIds.push(leaderTask.id);
